...
The 2 lines without coverage are relative to a try-catch block that is not triggered by any test, as it guards against an \texttt{ReflectiveOperationException} that cannot occur in the current implementation (line 85).

Similarly, there is 1 uncovered branch that is unreachable due to being covered in a public method. For example, the branch in \texttt{delete} that checks for a null node is not covered because the public \texttt{delete} method already guards against null nodes, so the internal method is never called with a null node.
