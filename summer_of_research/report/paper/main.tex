...
\begin{document}
    ...
    integrated pest management as alternatives or complements to synthetic pesticides. Their practical use depends on the crop, target organism, microbial strain,
    formulation, application conditions, and regulatory context. Microbial agents can act through several mechanisms,
    including plant defence induction, competition, hyperparasitism, and antibiosis. These
    ...
    The research question is: how well can an authorisation-derived ranker retrieve a held-out microbial BCA for a
    crop--pathogen query in the current Portuguese catalogue, and does adding a small BCA co-occurrence signal improve
    retrieval over direct support alone? This question concerns recovery of observed authorisation associations. It does
    not assess biological efficacy, safety, or legal authorisation for a new use because the sources contain no
    independent biological labels.


    \section{Contributions}\label{sec:research-goals}

    This paper makes \textbf{one contribution}: an evaluation of a transparent authorisation-derived ranker. It tests
    whether the ranker recovers held-out BCA associations for crop--pathogen queries in warm-start contexts. The
    integration and triage stages support this evaluation by preserving traceable evidence and separating unresolved
    rows from accepted authorisation records. Product names remain evidence users can inspect, not ranked items.
    ...
    reference-list changes affect row counts.
