...
The Commercial BCA database phase in Figure~\ref{fig:pipeline} turns the shared records into evidence states while preserving the source fields needed for later inspection and ranking.
...
The database creation stage assigns each record one of three evidence states.
...
These are authorisation evidence states, not biological efficacy assessments: a discarded record means that the pipeline found no BCA signal, not that the product or microorganism is ineffective.
...
The final and review states also provide the BCA-related outcome used by the diagnostic screening model evaluated later in the paper; discarded provides the no-signal outcome.
